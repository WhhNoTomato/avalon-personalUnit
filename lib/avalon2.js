   ...
   /*     var welcomeIntro = ["%cavalon.js %c" + avalon.version + " %cin debug mode, %cmore...", "color: rgb(114, 157, 52); font-weight: normal;", "color: rgb(85, 85, 85); font-weight: normal;", "color: rgb(85, 85, 85); font-weight: normal;", "color: rgb(82, 140, 224); font-weight: normal; text-decoration: underline;"];
    ...
    avalon.directive('attr', {
        ...
        update: function update(vdom, value) {
            ...
            for (var i in value) {
                if(value[i] == undefined$1){
                    if(!isPlaceholer()){
                        if(vdom.props.placeholder!=undefined$1){
                            value[i] = vdom.props.placeholder;
                        }
                    }else{
                        value[i] = "";
                    }
    ...
    var phonenoExp = /^(13[0-9]|14[0-9]|15[0-9]|18[0-9]|17[0-9])\d{8}$/;
    ...
    valiDir.defaults = {
        ...
        onError: function (reasons) {
            reasons.forEach(function (reason) {
                document$1.getElementById(reason.element.id+'Tips').html(reason.getMessage()).css("color","red");
            })
        },
        onSuccess:  function (reasons) {
            reasons.forEach(function (reason) {
                document$1.getElementById(reason.element.id+'Tips').html("");
            })
        },
        onComplete: avalon.noop,
        onManual: avalon.noop,
        onReset: function (reasons) {
            $('#'+reasons.target.id+'Tips').html("");
        },
